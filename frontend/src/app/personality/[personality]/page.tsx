...
import SeriesList from './series_list'
...
export default async function Personality({ params }: { params: { personality: string } }) {
    ...
    return (
        <main className='bg-gradient-to-b from-azul-claro from-10% via-azul-claro via-40% to-stone-200 to-80% h-full flex flex-col justify-center items-center gap-5'>
            <h1 className={`${whiteStar.className} text-white p-2 mt-3 text-4xl md:text-6xl md:mt-10 text-center`}>Tu personalidad es</h1>
            <section className='bg-white flex flex-col m-2 flex-wrap p-2 rounded bg-opacity-15 gap-5 shadow-lg'>
                <article className='w-full mb-10 px-5 flex flex-col md:gap-10 items-center'>
                    <h2 className={`${whiteStar.className} text-white hover:bg-opacity-90 duration-300 rounded p-10 text-center text-6xl md:text-8xl`}>{params.personality}</h2>
                ...
                <section className='grid md:grid-cols-2 gap-2'>
                    <article className='mx-2 lg:mx-0 px-4 flex flex-col gap-4 bg-gray-100 bg-opacity-50 rounded py-5 shadow-lg h-min'>
                ...
                <section className='grid md:grid-cols-[4fr,1fr] gap-2'>
                    <article className='md:order-2 mx-2 px-4 flex flex-col gap-4 bg-gray-100 bg-opacity-50 rounded py-5 shadow-lg items-center h-min'>
                        <h2 className={`${whiteStar.className} p-4 bg-white bg-opacity-40 w-full rounded text-4xl text-center leading-loose`}>¿Te sientes identificadx?</h2>
                        <Rating />
                    </article>
                    <article className='md:order-1 mx-2 px-4 flex flex-col gap-4 bg-gray-100 bg-opacity-50 rounded py-5 shadow-lg'>
                        ...
                        <div className='gap-4 columns-2 md:columns-4 lg:columns-8 [&>div:not(:first-child)]:mt-8 bg-white bg-opacity-40 p-4 rounded'>
                            ...
                            <SeriesList series={series} personality={params.personality} />
                        </div>
                    </article>
                </section>
            </section>
